...
def main():
    ...
    parser.add_argument("--min-prob", type=float, default=MIN_PROBABILITY, help="Min probability (default: 0.90)")
    parser.add_argument("--max-prob", type=float, default=MAX_PROBABILITY, help="Max probability (default: 0.98)")
    ...
    print(f" Filter: Win Probability {args.min_prob*100:.0f}% - {args.max_prob*100:.0f}% | Tight Spreads | Net +EV")
    print("="*80)
    ...
    candidates = scanner.scan_all_opportunities()
    ...
    for i, c in enumerate(candidates[:args.top], 1):
        print(f"\n[{i:02d}] {c['ticker']} -> {c['action']} @ ${c['ask_price']:.2f} ({c['implied_prob']} Prob)")
